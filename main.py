...
def thread_reproduce(reproducer, protocol, signal):
    ...
    time.sleep(1)  # there seems to be a half a second mute recording on the listener
...
if __name__ == '__main__':
    ...
    for name, signal in signals.items():
        ...
        listener_string += ' --filename ' + name + '.wav'
        ...
        print('correctly heard', amount_of_ones(correctly_heard), 'not heard', amount_of_ones(not_heard),
              'wrongly heard', amount_of_ones(wrongly_heard), 'total amount', 8 * len(signal))
